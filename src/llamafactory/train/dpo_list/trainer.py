...
class CustomDPOTrainer(DPOTrainer):
    ...
    def dpo_loss(
        ...
        if self.list_dpo_method == "test":
            ...
            # p2 = torch.clamp(p2, min=1e-3)
            logits_p2 = torch.log(p2)  
            losses = -(logits_p1 + logits_p2 * 2)

        elif self.list_dpo_method == "v5":
            chosen_logratios = chosen_logratios.to(self.accelerator.device)
            middle_logratios = middle_logratios.to(self.accelerator.device)
            rejected_logratios = rejected_logratios.to(self.accelerator.device)
            ...
            # p2 = torch.clamp(p2, min=1e-3)
            logits_p2 = torch.log(p2)  
            losses = -(logits_p1 + logits_p2 * 5)

        elif self.list_dpo_method == "v6":
            chosen_logratios = chosen_logratios.to(self.accelerator.device)
            middle_logratios = middle_logratios.to(self.accelerator.device)
            rejected_logratios = rejected_logratios.to(self.accelerator.device)
            ...
            # p2 = torch.clamp(p2, min=1e-3)
            logits_p2 = torch.log(p2)  
            losses = -(logits_p1 * 2 + logits_p2)

        elif self.list_dpo_method == "v7":
            chosen_logratios = chosen_logratios.to(self.accelerator.device)
            middle_logratios = middle_logratios.to(self.accelerator.device)
            rejected_logratios = rejected_logratios.to(self.accelerator.device)
            ...
            # p2 = torch.clamp(p2, min=1e-3)
            logits_p2 = torch.log(p2)  
            losses = -(logits_p1 + logits_p2 * 10)

        elif self.list_dpo_method == "v10":
            chosen_logratios = chosen_logratios.to(self.accelerator.device)
            middle_logratios = middle_logratios.to(self.accelerator.device)
            rejected_logratios = rejected_logratios.to(self.accelerator.device)
            ...
            # p2 = torch.clamp(p2, min=1e-3)
            logits_p2 = torch.log(p2)  
            losses = -(logits_p1 + logits_p2 * 15)

        elif self.list_dpo_method == "v11":
            chosen_logratios = chosen_logratios.to(self.accelerator.device)
            middle_logratios = middle_logratios.to(self.accelerator.device)
            rejected_logratios = rejected_logratios.to(self.accelerator.device)
